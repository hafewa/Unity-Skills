...
| `uitk_create_batch` | `[{type, savePath, content?, ussPath?}]` type: uss/uxml |
...
| Skill | Parameters |
|-------|------------|
| `uitk_create_uss` | savePath, content? |
| `uitk_create_uxml` | savePath, content?, ussPath? |
| `uitk_read_file` | filePath → path, type, lines, content |
| `uitk_write_file` | filePath, content |
| `uitk_delete_file` | filePath |
| `uitk_find_files` | type?(uss/uxml/all), folder?, filter?, limit? |
| `uitk_create_document` | name?, uxmlPath?, panelSettingsPath?, sortOrder?, parentName?, parentInstanceId?, parentPath? |
| `uitk_set_document` | name?/instanceId?/path?, uxmlPath?, panelSettingsPath?, sortOrder? |
| `uitk_create_panel_settings` | savePath, scaleMode?, referenceResolutionX?, referenceResolutionY?, screenMatchMode? |
| `uitk_list_documents` | — → documents[] |
| `uitk_inspect_uxml` | filePath, depth? → hierarchy tree |
| `uitk_create_from_template` | template, savePath, name? → ussPath, uxmlPath |
| `uitk_create_batch` | items: JSON array [{type,savePath,content?,ussPath?}] |

template: menu, hud, dialog, settings, inventory, list | scaleMode: ConstantPixelSize, ScaleWithScreenSize, ConstantPhysicalSize
